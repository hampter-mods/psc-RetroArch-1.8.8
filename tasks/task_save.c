...
bool content_save_state(const char *path, bool save_to_disk, bool autosave)
{
   ...
   if (save_to_disk)
   {
      if (autosave)
      {
         ...
         bool thumbnail_enable       = settings->bools.savestate_thumbnail_enable;
#if defined(HAVE_ZLIB)
         bool compress_files         = settings->bools.savestate_file_compression;
#endif
...
#if defined(HAVE_ZLIB)
         if (compress_files)
            save_success = rzipstream_write_file(path, data, info.size);
         else
#endif
            save_success = filestream_write_file(path, data, info.size);
         ...
         if (thumbnail_enable)
            if (!take_screenshot(settings->paths.directory_screenshot,
                     ...
                     false,
                     false))
               RARCH_WARN("Failed to save auto savestate thumbnail: \"%s.png\".\n",
                     path);
