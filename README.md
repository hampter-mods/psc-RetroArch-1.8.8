# PSC Store RetroArch 1.8.8 Source

This repository contains the RetroArch 1.8.8 source tree used for the patched
RetroArch binary distributed with PSC Store.

PSC Store release repository:

https://github.com/hampter-mods/pscstore-release

Corresponding PSC Store binary:

`/media/project_eris/opt/retroarch/retroarch`

## Purpose

This source is provided so recipients of PSC Store can inspect and rebuild the
patched RetroArch 1.8.8 binary that PSC Store distributes for Project Eris /
PS Classic systems.

The PSC Store build is based on RetroArch 1.8.8 and includes PSC Store /
Project Eris-oriented changes for the disc-project workflow, including fixes
around resume point and save-state loading behavior when used with the
disc-project PCSX-ReARMed core.

## Building

The included PSC-oriented build helper is:

```sh
PACK_UPX=1 ./docker-build-retroarch.sh
```

That script builds inside a Debian Stretch armhf container so the resulting
binary remains compatible with the Project Eris / PS Classic runtime
environment. Set `DOCKER_REBUILD_IMAGE=1` if the local build image needs to be
rebuilt.

## License

RetroArch is licensed under the GNU General Public License. This repository
keeps the upstream RetroArch license files, including `COPYING` and `LICENSE`.
PSC Store does not relicense RetroArch.

## Upstream

Upstream RetroArch project:
