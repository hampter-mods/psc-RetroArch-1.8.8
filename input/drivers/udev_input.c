...
static bool udev_device_name_contains(int fd, const char *needle)
{
   char name[256];

   if (fd < 0 || string_is_empty(needle))
      return false;
...
static bool udev_is_gpio_keys_devnode(const char *devnode)
{
   int fd;
   bool is_gpio = false;

   if (string_is_empty(devnode))
      return false;
   ...
   is_gpio = udev_device_name_contains(fd, "gpio-keys");
   close(fd);
...
static bool udev_handle_front_panel_key(const struct input_event *event)
{
   if (!event || event->type != EV_KEY || event->value != 1)
      return false;

#ifdef KEY_SLEEP
   if (event->code == KEY_SLEEP)
   {
      RARCH_LOG("[udev]: Front panel KEY_SLEEP ignored.\n");
      return true;
   }
#endif
   ...
   {
      RARCH_LOG("[udev]: Front panel KEY_PLAYPAUSE -> quit.\n");
   ...
   {
      RARCH_LOG("[udev]: Front panel KEY_RESTART -> quit.\n");
      command_event(CMD_EVENT_QUIT, NULL);
      return true;
   }
#endif
   ...
   {
      RARCH_LOG("[udev]: Front panel KEY_POWER -> quit.\n");
      command_event(CMD_EVENT_QUIT, NULL);
      return true;
   }
#endif
...
#ifdef KEY_EJECTCD
   if (event->code == KEY_EJECTCD)
   {
      RARCH_LOG("[udev]: Front panel KEY_EJECTCD -> menu toggle.\n");
      ...
      const struct input_event *event, udev_input_device_t *dev)
{
   ...
   {
      case EV_KEY:
         if (udev_handle_front_panel_key(event))
            return;

         keysym = input_unify_ev_key_code(event->code);
         if (keysym == KEY_RESERVED)
            return;
...
static bool udev_has_devnode(udev_input_t *udev, const char *devnode)
{
   unsigned i;
   ...
   for (i = 0; i < udev->num_devices; i++)
      if (string_is_equal(udev->devices[i]->devnode, devnode))
         return true;
      ...
      enum udev_input_dev_type type, const char *devnode, device_handle_cb cb)
{
   ...
   if (type == UDEV_INPUT_KEYBOARD && udev_device_name_contains(fd, "gpio-keys"))
   {
      grab = 1;
      if (ioctl(fd, EVIOCGRAB, grab) != 0)
         RARCH_WARN("[udev]: Failed to grab gpio-keys device %s (%s).\n",
               devnode, strerror(errno));
      else
         RARCH_LOG("[udev]: Grabbed gpio-keys device %s.\n", devnode);
   }
...
static void udev_input_handle_hotplug(udev_input_t *udev)
{
   ...
   else if (devnode && udev_is_gpio_keys_devnode(devnode))
   {
      dev_type   = UDEV_INPUT_KEYBOARD;
      cb         = udev_handle_keyboard;
   }
   ...
   {
      if (!udev_has_devnode(udev, devnode))
      {
      ...
      enum udev_input_dev_type type, device_handle_cb cb)
{
   ...
   for (item = devs; item; item = udev_list_entry_get_next(item))
   {
      ...
      if (devnode)
      {
         bool exists = udev_has_devnode(udev, devnode);
         int fd = open(devnode, O_RDONLY | O_NONBLOCK);

         if (fd != -1)
         {
            if (!exists && !udev_input_add_device(udev, type, devnode, cb))
   ...
   /* Some keyboard-like input sources (e.g. gpio-keys on PSC) expose
    * ID_INPUT_KEY=1 without ID_INPUT_KEYBOARD=1. */
   type_str_alt = "ID_INPUT_KEY";
   enumerate = udev_enumerate_new(udev->udev);
   if (!enumerate)
      return true;

   udev_enumerate_add_match_property(enumerate, type_str_alt, "1");
   udev_enumerate_scan_devices(enumerate);
   devs = udev_enumerate_get_list_entry(enumerate);
   ...
   for (item = devs; item; item = udev_list_entry_get_next(item))
   {
      ...
      if (devnode && !udev_has_devnode(udev, devnode))
      {
         int fd = open(devnode, O_RDONLY | O_NONBLOCK);
   ...
   for (item = devs; item; item = udev_list_entry_get_next(item))
   {
      ...
      if (devnode &&
          !udev_has_devnode(udev, devnode) &&
          udev_is_gpio_keys_devnode(devnode))
      {
         if (!udev_input_add_device(udev, type, devnode, cb))
            RARCH_ERR("[udev] Failed to open gpio-keys device: %s (%s).\n",
                  devnode, strerror(errno));
         else
            RARCH_LOG("[udev]: Keyboard #%d (%s) [gpio-keys].\n",
                  device_index++, devnode);
      }
