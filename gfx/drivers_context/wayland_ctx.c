...
typedef struct gfx_ctx_wayland_data
{
   ...
   struct wl_shell_surface *wl_shell_surf;
   ...
   if (state == WL_POINTER_BUTTON_STATE_PRESSED)
   {
      if (button == BTN_LEFT)
      {
         ...
         if (BIT_GET(wl->input.key_state, KEY_LEFTALT))
         {
            ...
            else if (wl->wl_shell_surf)
               wl_shell_surface_move(wl->wl_shell_surf, wl->seat, serial);
         }
      }
...
static void handle_wl_shell_surface_ping(void *data,
      struct wl_shell_surface *shell_surface, uint32_t serial)
{
   wl_shell_surface_pong(shell_surface, serial);
}

static void handle_wl_shell_surface_configure(void *data,
...
static void handle_wl_shell_surface_popup_done(void *data,
...
static void gfx_ctx_wl_destroy_resources(gfx_ctx_wayland_data_t *wl)
{
   ...
   if (wl->wl_shell_surf)
      wl_shell_surface_destroy(wl->wl_shell_surf);
...
static void gfx_ctx_wl_update_title(void *data)
{
   ...
   if (wl && title[0])
   {
      ...
      if (wl->xdg_toplevel)
         ...
         wl_shell_surface_set_title(wl->wl_shell_surf, title);
   }
}
...
static void *gfx_ctx_wl_init(void *video_driver)
{
   ...
   if (!wl->xdg_shell && !wl->zxdg_shell && wl->wl_shell)
   {
      ...
      bool fullscreen)
{
   ...
   {
      wl->wl_shell_surf = wl_shell_get_shell_surface(wl->wl_shell, wl->surface);
      if (!wl->wl_shell_surf)
         goto error;

      wl_shell_surface_add_listener(
            wl->wl_shell_surf, &wl_shell_surface_listener, wl);
      wl_shell_surface_set_toplevel(wl->wl_shell_surf);
      wl_surface_commit(wl->surface);
      wl_display_roundtrip(wl->input.dpy);
   }
   ...
   if (fullscreen)
   {
	   if (wl->xdg_toplevel)
         ...
         wl_shell_surface_set_fullscreen(wl->wl_shell_surf,
